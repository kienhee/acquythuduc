...
class AuthController extends Controller
{
    ...
    public function SendMailForgotPassword(Request $request)
    {
        ...
        if ($user) {
            ...
            $checkInsertTokenToDB = PasswordRessetTokens::insert(["email" => $email, "token" => $token]);
            ...
            Mail::to($user->email)->send(new ResetPassword($name, $email, $team, $address, $url));
            session()->flash(
                'success',
    ...
    public function resetPassword(Request $request)
    {
        ...
        if ($request->has('email') && $request->has('token')) {
            ...
            if ($checkToken && $checkToken->token == $request->token) {
                $layout = 'auth';
