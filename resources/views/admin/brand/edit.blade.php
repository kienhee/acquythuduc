...
@section('content')
    <x-breadcrumb parentName="Partner" parentLink="dashboard.brand.index" childrenName="Update Partner" />
    <div class="row">
        <div class="col-md-12">
            <div class="card mb-4">
                <x-alert />
                <x-header-table tableName="Update Partner" link="dashboard.brand.index" linkName="All Partners" />
                <div class="card-body">
                    <form action="{{ route('dashboard.brand.update', $brand->id) }}" method="POST"
                        ...
                        <div class="row">
                            <div class="d-flex flex-column align-items-center justify-content-center gap-4 mb-3">
                                <img src="{{ $brand->logo ?? asset('images/upload.png') }}" alt="Logo"
                                ...
                                <div class="button-wrapper text-center">
                                    <label for="upload" class="btn btn-primary me-2 mb-4" tabindex="0">
                                        ...
                                        <input type="hidden" name="cover" value="{{ $brand->logo }}">
                                    </label>
                                </div>
                                @error('logo')
                            ...
                            <div class="mb-3 col-md-12">
                                ...
                                <input class="form-control @error('name') is-invalid @enderror" type="text"
                                    id="name" name="name" value="{{ $brand->name ?? old('name') }}" autofocus />
                                @error('name')
                                    <p class="text-danger my-1">{{ $message }}</p>
                                @enderror
                            </div>
